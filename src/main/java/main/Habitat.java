...
public class Habitat extends Application {
    ...
    public void start(Stage stage) {
        ...
        copyRight.setTextFill(Color.rgb(240, 240, 240));
        copyRight.relocate(50, 340);
        ...
        AntDB.getInstance().getLoadBtn().setOnAction(e -> {
            try {
                AntDB.getInstance().loadIntoDB();
            } catch (SQLException ex) {
                System.err.println("Ошибка: " + ex.getMessage());
            }
            Platform.runLater(() -> {
                for (Object ant : AntCollection.ants().antVector().stream().
                        map(AbstractAnt::getVisualObject).toArray()) {
                    if (!antRoot.getChildren().contains(ant))
                        antRoot.getChildren().add((Circle)ant);
                }
            });
        });
        ...
        controls.getChildren().addAll(
                ...
                AntDB.getInstance().getLoadBtn(), AntDB.getInstance().getSaveBtn(),
                AntDB.getInstance().getSaveWar(),AntDB.getInstance().getSaveWork());
